...
from ..schema.calculated_schema import CalculatedSchemaCreate, CalculatedSchemaUpdate, CalculatedSchemaResponse

from .fields_of_view import FIELDS_OF_VIEW_PATTERN

router = APIRouter(prefix="/calculated", tags=["Calculated"])

@router.get("/get_calculates", response_model=List[CalculatedSchemaResponse], description="Получение данных о всех Calculated параметров")
async def get_calculates(db: AsyncSession = Depends(get_db)):
    try:
        ...
        stmt = select(
            ...
            FirstParameter, Calculated.parameter_1_id == FirstParameter.id
        ).join(
            ResultParameter, Calculated.result_param_id == ResultParameter.id
        ).join(
            SecondParameter, Calculated.parameter_2_id == SecondParameter.id
        )
        result = await db.execute(stmt)
        calculates = result.scalars().all()
        if not calculates:
            return res
...
@router.get("/get_calculate/{id}", response_model=CalculatedSchemaResponse, description="Получение данных о Calculated по id записи")
...
@router.post("/add_param/{param_id}", response_model=CalculatedSchemaResponse, description="Создание записи в Calculated")
...
@router.put("/update/{node_id}", response_model=CalculatedSchemaResponse, description="Занесение/обновление данных в таблицу Calculated")
async def update(
    node_id: int,
    ...
    try:
        ...
        # Сборка шаблона
        stmt = select(
            FirstParameter.name.label('parameter_1_name'), ResultParameter.name.label('result_param_name'),
            SecondParameter.name.label('parameter_2_name')
        ).join(FirstParameter, FirstParameter.id == existing_node.parameter_1_id).join(
            ResultParameter, ResultParameter.id == existing_node.result_param_id).join(
        ...
        data = {
            ...
            'parameter_1_name': param.parameter_1_name,
            'parameter_2_id': existing_node.parameter_2_id,
            'parameter_2_name': param.parameter_2_name,
