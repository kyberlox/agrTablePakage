...
from ..schema.selected_file_schema import SelectedFileSchemaCreate, SelectedFileSchemaUpdate, SelectedFileSchemaResponse
    ...
    try:
        ...
        for field in FIELDS_OF_VIEW_PATTERN['selected_file']['fields']:
            if field['field'] in data and data[field['field']] is not None:
                field['value'] = selected_file[field['field']]
...
@router.put("/upload/{node_id}", response_model=SelectedFileSchemaResponse, description="Загрузка файла к параметру")
async def upload(
    node_id: int,
    sel_file: UploadFile,
    db: AsyncSession = Depends(get_db)
...
@router.post("/add_param/{param_id}", response_model=SelectedFileSchemaResponse, description="Создание записи для загрузки файла")
...
@router.delete("/delete_file/{node_id}", response_model=SelectedFileSchemaResponse, description="Удаление файла")  
async def delete_file(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Ошибка при удалении файла в SelectedFile с id: {node_id}")

@router.delete("/delete_node/{node_id}", response_model=SelectedFileSchemaResponse, description="Удаление записи с SelectedFile")  
...
@router.get("/get_selected_file/{node_id}", response_model=SelectedFileSchemaResponse, description="Получение данных о загруженном файле к параметру")
...
@router.get("/get_selected_files", response_model=List[SelectedFileSchemaResponse], description="Получение данных о всех загруженных файлах")
